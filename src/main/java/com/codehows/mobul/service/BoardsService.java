...
//                .orElseThrow(EntityNotFoundException::new);
    ...
    public Long saveBoard(BoardsFormDTO boardsFormDTO, List<MultipartFile> fileList, HttpSession session) throws Exception {
        ...
        boards.setUsers(users); // 로그인한 사용자의 아이디 정보를 게시글 엔티티에 저장
    ...
    public void updateView(Long boardId) {
        boardsRepository.updateBoardView(boardId);
    }

/*
    @Transactional
    public BoardsDTO findByBoardId(Long boardId){
        Optional<Boards> optionalBoardsEntity = boardsRepository.findByBoardId(boardId);
