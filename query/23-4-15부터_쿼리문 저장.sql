...
-- 프라이머리키 두개 다 설정하는 방법 
alter table user_role ADD primary key(user_role_id, role_id);

desc user_role;


-- role_user 테이블의 primary key 설정
ALTER TABLE user_role ADD PRIMARY KEY (user_role_id);

-- role_user 테이블과 users 테이블 간의 외래키 설정
ALTER TABLE user_role ADD CONSTRAINT fk_role_user_id
    FOREIGN KEY (user_role_id) REFERENCES users (user_id);

-- role_user 테이블과 role 테이블 간의 외래키 설정
